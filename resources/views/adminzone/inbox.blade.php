    ...
    <table class=" text-white text-center">
        ...
        <tbody>           
                ...
                <tr class="border-b-red-600 border-b ">  
                    <td class="text-green-800 p-4"><a href="" class="border-2 p-2 border-green-500 rounded-full font-extrabold">New</a></td>
                ...
                <tr class="border-b-red-600 border-b text-gray-700">
                    <td class="p-5"></td>
                    <td class="border border-x-red-600 p-5 uppercase text-gray-600 font-bold @if ($suggestion->user->rol == 1) text-gray-400 @elseif ($suggestion->user->rol == 2) text-gray-600 @else text-gray-600 @endif">
                     @if ($suggestion->user->rol == 1)Registered @elseif ($suggestion->user->rol ==2) Admin @else Anonymou @endif</td>
                     @endif 
                    ...
                    <td class="p-5"><form action="{{route('suggestions.update')}}" method="post">
                        @csrf
                        @method("PUT")
                            ...
                            <!-- Botón de envío -->
                             <input type="submit" value="Read" class=" shadow-red-600 shadow-md border border-amber-400 bg-gray-500  hover:bg-sky-700 transition-colors cursor-pointer w-full p-2 text-slate-900 font-header rounded-lg">
                        </form></td>
                        <td class="p-5">
