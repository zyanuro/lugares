            ...
            <div>
                <tbody>
                    ...
                    @foreach ($users as $user)
                        ...
                        @elseif ($user->rol == 2) border border-green-600 text-green-600 @endif ">
                                @if ($user->rol === 0)
                                    Blocked
                                @elseif ($user->rol == 1)
                                    Registered
                            ...
                            <td class="@if ($user->name == 'Anonymous') hidden @endif">
                                <form action="{{ url('adminzone/users/update', $user->id) }}" method="post">
                                    @csrf
                                    @method('PUT')
                                    <!-- Campo de entrada oculto -->
                                    <select id="rol" name="rol"
                                        class="text-red-600 font-bold bg-slate-300 rounded-full  w-2/3 my-1">

                                        <option value="2">- Select -</option>
                                        <option value="0" class="@if ($user->rol === 0) hidden @endif">Block
                                            User</option>
                                        <option value="1" class="@if ($user->rol == 1) hidden @endif">
                                            Convert Registered</option>
                                        <option value="2" class="@if ($user->rol == 2) hidden @endif">Make
                                            admin</option>
                                    </select>
                                    <!-- Botón de envío -->
                                    <input type="submit" value="Done"
                                        class=" shadow-red-600 shadow-md border border-amber-400 bg-slate-500 hover:bg-sky-700 transition-colors cursor-pointer w-auto p-2 text-slate-900 font-sans font-bold rounded-full">
                                </form>
                            </td>

                            <td class="p-5 @if ($user->name == 'Anonymous') hidden @endif">
                                <form action="{{ url('adminzone/users/' . $user->id) }}" method="post" name="delete"
                                    class="my-3">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit"
