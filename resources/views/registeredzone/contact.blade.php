        ...
        <div class="max-w-md mx-auto bg-gray-4 00 p-6 rounded-md shadow-md border-2 border-red-600 bg-gray-300">
            ...
            <form action="{{ url('suggestions') }}" method="GET" class="">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="suggestion" id="suggestion" rows="4" maxlength="250" class="w-full px-3 py-2 border rounded-md"
                        required>
                </textarea>
                </div>
                ...
                <div class="mb-5">
                    <label for="user_id" class="mb-2 block  text-gray-500 font-bold text-center">
                        <p class=" font-sans text-teal-600 border border-red-600 p-3 w-full rounded-lg">"User : @auth
                            {{ Auth::user()->email }} @endauth
                        </p>
                    </label>
                    <input id="user_id" name="user_id" type="text" placeholder=""
                        class="hidden border border-red-600 p-3 w-full rounded-lg"
                        @auth value="{{ Auth::user()->id }}" @endauth value='6' />
                </div>


                <div class="mt-6">
                    <button type="submit"
                        class="bg-gray-300 @if (!isset($msg)) visible @else invisible @endif                    
